...
namespace Pico.Platform.Samples.SimpleSample
{
    ...
    public class TestEntry : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            var testButton = GameObject.Find("TestButton");
            testButton.GetComponent<Button>().onClick.AddListener(() => { GetAccessToken(null); });
        }

        private void OnButtonClick()
        {
            var obj = GameObject.Find("Canvas/Input/InputField");
            var inputField = obj.GetComponent<SougouInputField>();
        ...
        string UserList2String(UserList userList)
        {
            ...
            builder.Append(
                $"UserList:Count{userList.Count} hasNext={userList.HasNextPage} bodyParams={userList.NextPageParam}");
        ...
        void LaunchFriendRequest(string[] args)
        {
            ...
            Users.LaunchFriendRequestFlow(args[1]).OnComplete(msg =>
            {
                if (msg.IsError)
                {
        ...
        void GetLoggedInUserFriends(string[] args)
        {
            ...
            Users.GetFriends().OnComplete(msg =>
            {
                if (msg.IsError)
                {
                    var err = msg.GetError();
        ...
        void GetUser(string[] args)
        {
            ...
            Users.Get(args[1]).OnComplete(msg =>
            {
                if (msg.IsError)
                {
                    var err = msg.GetError();
        ...
        void GetUserArrayNextPage(string[] args)
        {
            ...
            Users.GetNextUserListPage(cacheUserList).OnComplete(msg =>
            {
                if (msg.IsError)
                {
                    var err = msg.GetError();
